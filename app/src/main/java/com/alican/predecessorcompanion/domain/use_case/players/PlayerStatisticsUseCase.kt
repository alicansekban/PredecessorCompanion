...
class PlayerStatisticsUseCase @Inject constructor(
    private val playersRepository: PlayersRepository
) {
    operator fun invoke(playerId: String): Flow<UIState<PlayerStatisticsUIModel>> {
        return flow {
            emit(UIState.Loading())
            emit(
                when (val result = playersRepository.getPlayerStatistics(playerId)) {
